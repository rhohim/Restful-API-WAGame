...
class EditShareid(Resource):
    def put(self,name):
        dataUpdate = Share.query.filter(Share.sosmed == name).first()
        datasosmed = request.form.get('sosmed')
        datacount = request.form.get('count')
        if datasosmed:
            dataUpdate.sosmed = datasosmed
        if datacount:
            dataUpdate.count = datacount
        db.session.commit()
        return make_response(jsonify({"msg" : "edited"}),200)
...
api.add_resource(EditShareid,"/wa/edit/<name>", methods=["PUT"])
